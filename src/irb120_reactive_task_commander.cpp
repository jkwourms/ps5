...
string g_object_name("gear_part");
...
int main(int argc, char** argv) {
    ...
    int offset = 0.5; //how much the gear is adjusted if it's position is not the "goal" position
    double goal_x = 3.2; //the goal x position
    double goal_y = 2.4; //the goal y position
    ...
    //go to the left side of the gear part to prepare for the push. Did this by increasing the y position.
    flange_origin <<g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + 0.09, 0.005; 
...
flange_origin <<g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y, 0.005; 
    
    //Push the gear to the right by checking if the x and y positions align with the "goal" position.
	...
	flange_origin <<g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + offset, 0.005;
    else
	flange_origin <<g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - offset, 0.005;
